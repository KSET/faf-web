...
useEffect(() => {
  ...
  const updateEye = () => {
    ...
    pt.x = mouseRef.current.x;
    pt.y = mouseRef.current.y;
    ...
    const clamped = Math.min(distance, maxEyeMovement);

    const targetX =
      eyeCenterX + (deltaX / distance) * clamped;
    const targetY =
      eyeCenterY + (deltaY / distance) * clamped;
